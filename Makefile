...
# Reinicia la app por serial sin reset de hardware: interrumpe, entra al REPL
# normal y fuerza un soft reboot (corre main.py). Ver tools/restart.py.
RESTART = uv run --with pyserial python tools/restart.py $(PORT)
# Misma versión que el firmware del badge (MicroPython 1.23, mpy v6.3).
MPY_CROSS = uv tool run --from mpy-cross==1.23.0 mpy-cross

.PHONY: test assets preview build deploy font flash firmware qr push stats restart
